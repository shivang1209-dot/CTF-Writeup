...
The server outputs 10 numbers (e.g. 50, 12, 17, 35, 52, 42, 0, 70, 56, 91) and asks for the next 5 spins (space-separated). The challenge source is in [Resources/slot.py](Resources/slot.py).
...
The server sends **10 numbers**: `state_0 % 100` through `state_9 % 100`. We must predict the **next 5 spins**: `state_10 % 100` through `state_14 % 100`.
...
(Replace with your solver that connects, reads 10 values, recovers state, and sends the next 5.)
